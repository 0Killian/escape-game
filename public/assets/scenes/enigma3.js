class Enigma3 extends Phaser.Scene {
  constructor() {
    super("Enigma3");
    ...
    this.characters = [
      { 
        ...
        name: "Rose Bukater",
        description: "Jeune femme de la haute société, prisonnière de son destin. Elle change de position et d'identité au fil de l'histoire : de victime enfermée à femme libre. Son ambivalence initiale crée le doute (alliée ou obstacle ?) avant sa métamorphose complète."
      },
      { 
        key: "caledon", 
        file: "Caledon Hockley.jpg", 
        name: "Caledon Hockley",
        description: "Fiancé de Rose, homme d'affaires fortuné et représentant des conventions sociales. Il défend l'ordre établi et la rigidité de classe, empêchant la transgression initiale. Il teste la détermination de ceux qui veulent franchir les barrières sociales."
      },
      { 
        key: "fabrizio", 
        file: "Fabrizio.jpg", 
        name: "Fabrizio",
        description: "Compagnon fidèle de Jack, ami italien plein de vie. Il soutient le protagoniste dans les moments clés et partage son humanité, son humour et sa loyauté. Sa présence offre un soutien moral constant dans l'aventure."
      },
      { 
        key: "molly", 
        file: "Molly brown.jpg", 
        name: "Molly Brown",
        description: "Femme indépendante et bienveillante, elle soutient Jack et Rose contre les barrières sociales. Elle transmet courage et lucidité, guide pragmatique qui aide les protagonistes à voir plus clair dans leur situation."
      },
      { 
        key: "lovejoy", 
        file: "Spicer Lovejoy.jpg", 
        name: "Spicer Lovejoy",
        description: "Valet de Caledon, personnage manipulateur et cynique. Il introduit la ruse et la surveillance, brouillant les pistes et créant des obstacles par des moyens détournés. Moteur de tension dramatique par ses actions sournoises et imprévisibles."
      },
      { 
        key: "jack", 
        file: "téléchargé.jpg", 
        name: "Jack Dawson",
        description: "Jeune homme sans fortune, animé par la liberté et la passion. Personnage principal qui traverse l'épreuve du voyage, de l'amour impossible et du sacrifice. Il incarne le mouvement, le risque, et la transformation de soi face à l'adversité."
      }
    ];
  ...
  onGameUpdate(server, room, event) {
      ...
      room.enigma3.roles.forEach((characterKey, roleIndex) => {
        if (characterKey) {
          ...
          Object.keys(this.assignments).forEach(key => {
            if (this.assignments[key] === this.roles[roleIndex] && key !== characterKey) {
    ...
    if (room.enigma3 && room.enigma3.completed) {
      this.showMessage("🎉 Bravo ! Vous avez correctement identifié tous les archétypes narratifs !\n\nLa plupart des histoires suivent des structures universelles.", "#00FF00");
      
      // Passer à la scène suivante après 5 secondes
      this.time.delayedCall(5000, () => {
        server.changeScene("main");
      });
    } else if (event && event.kind === "enigma3:submit-result" && !room.enigma3.completed) {
      this.showMessage("❌ Ce n'est pas tout à fait ça. Réessayez !", "#FF0000");
    }
  }
  ...
  create() {
    ...
    const title = this.add.text(width / 2, 30, "LE CASTING NARRATIF", {
      fontSize: "32px",
      fontStyle: "bold",
      color: "#FFD700",
    ...
    const subtitle = this.add.text(width / 2, 58, "✨ Cliquez sur un personnage pour découvrir son archétype ✨", {
      fontSize: "14px",
      color: "#FFF",
      fontStyle: "italic",
      fontFamily: "Arial"
    ...
    const charsPerRow = 3;
    const totalCharsWidth = charsPerRow * charSize + (charsPerRow - 1) * charSpacing;
    ...
    this.characters.forEach((char, index) => {
      ...
      const x = charStartX - totalCharsWidth / 2 + charSize / 2 + col * (charSize + charSpacing);
      const y = charStartY + row * (charSize + charSpacing + 50);

      // Ombre portée
      const shadow = this.add.rectangle(x + 3, y + 3, charSize + 8, charSize + 8, 0x000000, 0.5);
      shadow.setDepth(-1);
      ...
      // Badge "i" pour info
      const infoBadge = this.add.circle(x + charSize / 2 - 10, y - charSize / 2 + 10, 12, 0x4A90E2, 1);
      infoBadge.setStrokeStyle(2, 0xFFFFFF);
      infoBadge.setDepth(2);
      
      const infoText = this.add.text(x + charSize / 2 - 10, y - charSize / 2 + 10, "i", {
        fontSize: "16px",
      ...
      const nameText = this.add.text(x, y + charSize / 2 + 22, char.name, {
        fontSize: "13px",
        fontStyle: "bold",
        color: "#FFD700",
        backgroundColor: "#000",
      ...
      img.on("pointerdown", (pointer) => {
        ...
        img.once("pointerup", (upPointer) => {
          const distance = Phaser.Math.Distance.Between(startX, startY, upPointer.upX, upPointer.upY);
          if (distance < 10) {
    ...
    const totalRoleHeight = this.roles.length * roleHeight + (this.roles.length - 1) * roleSpacing;
    ...
    this.roles.forEach((role, index) => {
      ...
      // Ombre de la zone
      const dropShadow = this.add.rectangle(roleStartX + 2, y + 2, roleWidth, roleHeight, 0x000000, 0.4);
      ...
      const innerBorder = this.add.rectangle(roleStartX, y, roleWidth - 8, roleHeight - 8);
      ...
      // Icône pour le rôle
      const roleIcon = this.add.text(roleStartX - roleWidth / 2 + 20, y, "🎭", {
        fontSize: "20px"
      }).setOrigin(0.5);
      roleIcon.setDepth(1);

      // Texte du rôle avec style moderne
      const roleText = this.add.text(roleStartX, y - 8, role, {
        fontSize: "17px",
        fontStyle: "bold",
        color: "#FFD700",
        fontFamily: "Arial"
      ...
      const assignedLabel = this.add.text(roleStartX, y + 18, "", {
        fontSize: "12px",
        fontStyle: "italic",
        color: "#00FF88",
    ...
    // Ombre du bouton
    const buttonShadow = this.add.rectangle(width / 2 + 4, buttonY + 4, 200, 55, 0x000000, 0.5);
    buttonShadow.setDepth(998);
    
    // Background du bouton avec gradient simulé
    const buttonBg1 = this.add.rectangle(width / 2, buttonY, 200, 55, 0x4A90E2, 1);
    buttonBg1.setStrokeStyle(3, 0xFFD700, 1);
    buttonBg1.setDepth(999);
    
    const buttonBg2 = this.add.rectangle(width / 2, buttonY - 12, 200, 28, 0x5AA0F2, 0.5);
    buttonBg2.setDepth(999);
    
    const submitButton = this.add.text(width / 2, buttonY, "✓ VÉRIFIER", {
      fontSize: "22px",
      fontStyle: "bold",
      color: "#FFF",
      fontFamily: "Arial Black"
    }).setOrigin(0.5, 0.5).setInteractive().setDepth(1000);
    ...
    submitButton.on("pointerover", () => {
      ...
      this.tweens.killTweensOf([buttonBg1, buttonBg2, submitButton, buttonShadow]);
      this.tweens.add({
  ...
  submitSolution() {
    ...
    const allAssigned = Object.values(this.assignments).every(role => role !== null);
    
    if (!allAssigned) {
      this.showMessage("⚠️ Vous devez assigner un personnage à chaque rôle !", "#FF0000");
      return;
    }
  ...
  showCharacterDescription(character) {
    ...
    const shadow = this.add.rectangle(panelX + 6, panelY + 6, panelWidth, panelHeight, 0x000000, 0.8);
    shadow.setDepth(2000);

    // Fond du panneau avec style moderne
    const background = this.add.rectangle(panelX, panelY, panelWidth, panelHeight, 0x1a1a2e, 1);
    background.setStrokeStyle(4, 0x4A90E2, 1);
    background.setDepth(2000);
    background.setInteractive();
    
    // Bordure intérieure
    const innerBorder = this.add.rectangle(panelX, panelY, panelWidth - 10, panelHeight - 10);
    innerBorder.setStrokeStyle(2, 0xFFD700, 0.5);
    innerBorder.setFillStyle(0x000000, 0);
    innerBorder.setDepth(2000);

    // Barre de titre
    const titleBar = this.add.rectangle(panelX, panelY - panelHeight / 2 + 40, panelWidth, 80, 0x2a2a3e, 0.9);
    titleBar.setDepth(2000);

    // Icône du personnage
    const icon = this.add.text(panelX - panelWidth / 2 + 40, panelY - panelHeight / 2 + 40, "🎬", {
      fontSize: "36px"
    }).setOrigin(0.5).setDepth(2001);

    // Titre (nom du personnage)
    const title = this.add.text(panelX, panelY - panelHeight / 2 + 40, character.name, {
      fontSize: "32px",
      fontStyle: "bold",
      color: "#FFD700",
      fontFamily: "Arial Black"
    ...
    const description = this.add.text(panelX, panelY + 20, character.description, {
      fontSize: "17px",
      color: "#E0E0E0",
      backgroundColor: "transparent",
    ...
    const closeBg = this.add.rectangle(panelX, panelY + panelHeight / 2 - 40, 180, 50, 0xFF4444, 1);
    closeBg.setStrokeStyle(3, 0xFFFFFF, 1);
    closeBg.setDepth(2001);
    closeBg.setInteractive();
    
    const closeButton = this.add.text(panelX, panelY + panelHeight / 2 - 40, "✕ FERMER", {
      fontSize: "22px",
      fontStyle: "bold",
      color: "#FFF",
      fontFamily: "Arial"
    ...
    const closePanel = () => {
      ...
      this.tweens.add({
        targets: [overlay, shadow, background, innerBorder, titleBar, icon, title, description, closeButton, closeBg],
        alpha: 0,
        scaleX: 0.8,
        scaleY: 0.8,
        duration: 200,
    ...
    [overlay, shadow, background, innerBorder, titleBar, icon, title, description, closeButton, closeBg].forEach(obj => {
      obj.setAlpha(0);
      obj.setScale(0.8);
    });
    ...
    this.descriptionPanel = { overlay, shadow, background, titleBar, icon, title, description, closeButton, closeBg };
  }
}
